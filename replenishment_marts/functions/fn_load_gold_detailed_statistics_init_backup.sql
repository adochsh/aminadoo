...
--changeset 60098727:create:fn_load_gold_detailed_statistics_init_backup
CREATE OR REPLACE FUNCTION fn_load_gold_detailed_statistics_init_backup(period_start date, period_end date)
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$
declare
...
raise notice '==================================== START =====================================';
    ...
    from gold_refcesh_ods.v_cdeentcde ecd
        join gold_refcesh_ods.v_fouccom fc on ecd.ecdccin = fc.fccccin and ecd.is_actual = '1' and fc.is_actual = '1'
    ...
    from gold_refgwr_ods.v_prpentprop pdc
    join gold_refgwr_ods.v_prpdetprop lpc on lpc.lpcnprop = pdc.pdcnprop and lpc.is_actual = '1'
    where pdc.is_actual = '1'
    ...
    from gold_refgwr_ods.v_prpentprop pdc
    join gold_refgwr_ods.v_prpdetprop lpc on lpc.lpcnprop = pdc.pdcnprop and lpc.is_actual = '1'
    join gold_refcesh_ods.v_cdeentcde ecd on ecd.ecdcomm1 = cast(pdc.pdcnprop as varchar) and ecd.is_actual = '1'
    left join gold_refcesh_ods.v_cdedetcde dcd on ecd.ecdcexcde = dcd.dcdcexcde and lpc.lpccinr = dcd.dcdcinr and dcd.is_actual = '1'
    ...
    select pdc.pdcsite as site,
    	   ...
    	   art.artcexr as item,
    	   1 as tech_case_no
    from gold_refcesh_ods.v_cdedetcde dcd
    	join gold_refcesh_ods.v_cdeentcde ecd on ecd.ecdcexcde = dcd.dcdcexcde and ecd.is_actual = '1'
    	join gold_refgwr_ods.v_prpentprop pdc on ecd.ecdcomm1 = cast(pdc.pdcnprop as varchar) and pdc.is_actual = '1'
    	left join gold_refgwr_ods.v_prpdetprop lpc on lpc.lpcnprop = pdc.pdcnprop and lpc.lpccinr = dcd.dcdcinr and ecd.is_actual = '1'
    	left join tmp_orders_link tol on pdc.pdcnprop = tol.key_number and tol.tech_case_no = 1
    	LEFT JOIN gold_refcesh_ods.v_artrac art ON dcd.DCDCINR = art.artcinr and art.is_actual = '1'
    where dcd.is_actual = '1' and lpc.lpccinr is null
    	and cast(pdc.pdcdcde as date) >= period_start and cast(pdc.pdcdcde as date) <= period_end
    group by pdc.pdcsite, pdc.pdcccin, pdc.pdcdcde, pdc.pdcnprop, pdc.pdctpro, pdc.pdccfin, pdc.pdcvalide,
    ...
    from gold_refcesh_ods.v_cdedetcde dcd
    join gold_refcesh_ods.v_cdeentcde ecd on ecd.ecdcexcde = dcd.dcdcexcde and ecd.is_actual = '1'
    left join gold_refgwr_ods.v_prpentprop pdc on ecd.ecdcomm1 = cast(pdc.pdcnprop as varchar) and pdc.is_actual = '1'
    join tmp_orders_link tol on ecd.ecdcincde = tol.key_number and tol.tech_case_no = 2
    LEFT JOIN gold_refcesh_ods.v_artrac art ON dcd.DCDCINR = art.artcinr and art.is_actual = '1'
    where dcd.is_actual = '1' and pdc.pdcnprop is null and ecd.ecdsite <> ecd.ecdcincde and ecd.ecdnvalo is null
    ...
    from gold_refgwr_ods.v_prpentprop pdc
    left join gold_refgwr_ods.v_prpdetprop lpc on lpc.lpcnprop = pdc.pdcnprop and lpc.is_actual = '1'
    left join gold_refcesh_ods.v_cdeentcde ecd on ecd.ecdcomm1 = cast(pdc.pdcnprop as varchar) and ecd.is_actual = '1'
    left join tmp_corr_detailed t on pdc.pdcnprop = t.prop_number
    LEFT JOIN gold_refcesh_ods.v_artrac art ON lpc.LPCCINR = art.artcinr and art.is_actual = '1'
    where pdc.is_actual = '1' and ecd.ecdcomm1 is null and t.prop_number is null
    and pdc.pdcvalide = 1
    and cast(pdc.pdcdcde as date) >= period_start and cast(pdc.pdcdcde as date) <= period_end;
    ...
    from tmp_corr_detailed tcd
    join gold_refcesh_ods.v_cdedetcde dcd on coalesce(tcd.contract_numbers -> 'LS', tcd.contract_numbers -> 'EM',
    ...
    where tcd.tech_case_no = 2 and dcd.is_actual = '1' and ara.is_actual = '1' and sps.is_actual = '1'
