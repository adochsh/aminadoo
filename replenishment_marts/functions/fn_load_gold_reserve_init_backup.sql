...
--changeset 60098727:create:function:fn_load_gold_reserve_init_backup

CREATE OR REPLACE FUNCTION fn_load_gold_reserve_init_backup(cal_date date)
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$
declare
...
begin
    ...
    WHERE coalesce(app.APPAUTO,0) <> 1 and extract(year from cal_date) = extract(year from app.appddeb)
    ...
    SELECT tgr.code_lm,
	   ...
	   tgr.reserve_type,
	   tgr.reserve_code,
	   case when count(dppvdnsem) = 0 then null
            else round(((sum(coalesce(fdp.dppvdcorr, coalesce(fdp.dppvdcalc, 0))) / (count(fdp.dppvdnsem) * 7) ) * tgr.qty),2) end as qty_per_day,
	   tgr.start_date,
	   tgr.end_date,
	   tgr.store_num,
	   tgr.qty,
    ...
    LEFT JOIN gold_refgwr_ods.v_fctentpvh_dp fep ON dppvesite = tgr.store_num AND dppvecinl = tgr.code_logistics and fep.is_actual = '1'
    LEFT JOIN gold_refgwr_ods.v_fctdetpvh_dp fdp on fep.dppveid = fdp.dppvdeid AND fdp.DPPVDNSEM BETWEEN fcs1.SEMNSEM AND fcs2.SEMNSEM and fdp.is_actual = '1'
    ...
    WHERE coalesce(app.APPAUTO,0) = 1
        and extract(year from cal_date) = extract(year from app.appddeb)
        and app.is_actual = '1';
    ...
    cast(coalesce(ROUND(SUM(CASE extract(day from cas.CASDATE - sem.SEMDDEB)
    						...
    						WHEN 4 THEN REPCJ05
                            WHEN 5 THEN REPCJ06
                            ELSE REPCJ07 END  * COALESCE(DPPVDCALC, 0)),
                           3),
    ...
    LEFT JOIN gold_refgwr_ods.v_FCTENTPVH_DP fep ON fep.DPPVESITE = cas.CASSITE AND fep.DPPVECINL = coalesce(lf.inlr, app.APPCINL) and fep.is_actual = '1'
    LEFT JOIN gold_refgwr_ods.v_FCTDETPVH_DP fdp ON fep.dppveid = fdp.DPPVDEID AND sem.SEMNSEM = fdp.DPPVDNSEM and fep.is_actual = '1'
    WHERE app.appauto = 1 and app.is_actual = '1' and extract(year from cal_date) = extract(year from app.appddeb)
  ...
  INSERT INTO tmp_gold_reserve
    ...
    SELECT tgr.code_lm,
	   ...
	   tgr.store_num,
	   case when count(dppvdnsem) = 0 then null
       	    else  round(((sum(coalesce(fdp.dppvdcorr, coalesce(dppvdcalc, 0))) / (count(fdp.dppvdnsem) * 7) ) * aut.days_forecast),2) end as qty_per_day,
	   current_timestamp
    FROM tmp_gold_reserve_raw tgr
	...
	delete from gold_reserve
	where extract(year from start_date) = extract(year from cal_date);
